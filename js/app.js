...
import {
  ...
  packBus, unpackBus, isBus, busLabel, encodeShare, decodeShare
} from './util.js';
import {
...
// Serialise the live circuit into a save object. `forShare` produces a slim
// payload for a share link: it drops the built-in subcircuit defs (they're
// regenerated by registerBuiltinSubcircuits on load) and the bookkeeping fields
// (nextCompId/nextWireId/tick are recomputed/defaulted on load) — keeping a
// shared URL short. The full file-save keeps everything for an exact round-trip.
function serializeCircuit({ forShare = false } = {}) {
  // Deep-copy comps but strip non-serializable subScope (rebuilt on load).
  const cleanComps = comps.map(c => {
    const cc = { ...c, state: deepClone(c.state) };
    delete cc.subScope;
    return cc;
  });
  let subs = subcircuitDefs;
  if (forShare) {
    subs = {};
    for (const name in subcircuitDefs) if (!isBuiltinSubcircuit(name)) subs[name] = subcircuitDefs[name];
  }
  const data = { version: SAVE_FORMAT_VERSION, comps: cleanComps, wires, view,
                 subcircuitDefs: subs, customGates };
  if (!forShare) { data.nextCompId = nextCompId; data.nextWireId = nextWireId; data.tick = tick; }
  return data;
}

// Apply a loaded save object to the live world: migrate it forward (older →
// current via upgradeSave; a newer-than-us file is passed through), then install
// comps/wires/defs and re-settle. Throws if migration fails (caller catches).
...
document.getElementById('file-input').addEventListener('change', (e) => {
  ...
  reader.onload = () => {
    try {
      ...
      // A newer-than-us file may use fields we don't know; warn but allow.
...
// I3 — Shareable circuits. Encode the current circuit into the URL hash
...
function shareCircuit() {
  try {
    ...
    const big = url.length > 8000
      ? ' (note: this is a large link — it may be truncated by some chat/forum software)'
      : '';
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(url)
        .then(() => setStatus('Share link copied to clipboard' + big))
        .catch(() => setStatus('Share link is in the address bar (clipboard blocked)' + big));
    } else {
      setStatus('Share link is in the address bar' + big);
    }
